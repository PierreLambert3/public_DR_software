...
def populate_hparams(hparams, N):
    ...
    if not 'n iter' in hparams:
        hparams['n iter'] = 3000 # higher values bring better results, 1000 is usually enough

    if not 'LR' in hparams:
        # hparams['LR'] = max(5., 0.005*N)
        hparams['LR'] = 500.


def run_SQuaD_MDS(Xhd, hparams, progress_stuff=None):
    N, M = Xhd.shape
    ...
    decay_start = int(0.5 * N_iter)  # if random init: it is recommended to start the decay later to give the time for the global config to adjust with big steps
    if decay_start > 0:
